...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        #print node
        if problem.goal_test(fringe, node.state): 
            return node
        if node.state not in closed:
            closed[node.state] = True
            fringe.extend(node.expand(problem))    
    log.info('Open queue became empty. No solution found')
    return None

def ilp_lattice_search(problem, opened, closed):
    """Search through the successors of a problem to find a goal.
    The argument fringe should be an empty queue.
    If two paths reach a state, only use the best one.
    Based on alg from CProgol manual"""
    
    import copy
    import gc
    import base.utils.psutil as psutil
    
    #gco = len(gc.get_objects())
    #apm = mem_avail()
    apm = psutil.avail_phymem()/(1024*1024)
    #log.info(YBT + 'AVAIL_PHY_MEM before search: ' + repr(gco) + '  ' + BBB + repr(apm) + RESET)
    closed_dict = {}
    for hyp in problem.initial:
        opened.append(Node(hyp))
    
    while opened:
        node = opened.pop()
        problem.f(node)
        if node.state not in closed_dict:
            closed_dict[node.state] = True
            closed.append(node)
            opened.extend(node.expand(problem))    
            #log.info(YBB + 'Length of CLOSED: ' + RESET + repr(len(closed.A)))
            #log.info(YBB + 'Length of OPENED: ' + RESET + repr(len(opened.A)))
        # Get last element from closed, no pop as we don't want to delete that node
        best_node = closed.A[-1]
        if not best_node[1].state.goal_tested and len(closed.A) > 1:
            # If the len(closed.A) is 1 means best_node is initial hyp, so no need to test if it is goal.
            if problem.goal_test(opened, best_node[1].state): 
                sol = copy.deepcopy(best_node[1])          
                return (sol, NORMAL)

            #apm = psutil.avail_phymem()/(1024*1024)
            #apm = mem_avail()
            if apm < 200 or len(closed.A) > 7000:
            #if len(closed.A) > 5000:
                sol = copy.deepcopy(best_node[1])               
                log.warn(RBB + 'Possibiblity of "Out of memory error", returning the best available hypothesis' + RESET)
                #gco = len(gc.get_objects())
                #apm = mem_avail()
                #log.debug(YBT + 'AVAIL_PHY_MEM before search: ' + repr(gco) + '  ' + BBB + repr(apm) + RESET)
                del closed
                del opened
                log.info(YBB + 'BEST HYPOTHESIS: ' + RESET)
                log.info(repr(sol.state))
                return (sol, OUT_OF_MEMORY)
            
            closed.A[-1][1].state.goal_tested = True
            log.debug(repr(best_node[1].state.body) + ' [' + repr(best_node[1].state.score[0]) + ' ]')
    #gco = len(gc.get_objects())
    #apm = mem_avail()
    #apm = psutil.avail_phymem()/(1024*1024)
    #log.info(YBT + 'AVAIL_PHY_MEM after search: ' + repr(gco) + '  ' + BBB + repr(apm) + RESET)
    log.info('End of search, no candidate hypotheses left, returning the best so far.')    
    log.info(YBB + 'BEST HYPOTHESIS: ' + RESET)
    log.info(repr(closed.A[-1][1].state))
    
    # Return the best so far found and delete opened and closed to save memory
    sol = copy.deepcopy(closed.A[-1][1])
    del closed
    del opened
    return (sol, NORMAL)
...
def ilp_best_first_graph_search(problem, f):
    """Search the nodes with the lowest f scores first.
    You specify the function f(node) that you want to minimize; for example,
    if f is a heuristic estimate to the goal, then we have greedy best
    first search; if f is node.depth then we have depth-first search.
    ...
    f = memoize(f, 'f')
    # return graph_search(problem, PriorityQueue(min, f))
    #sol = graph_search(problem, PriorityQueue(max, f))
    sol = ilp_lattice_search(problem, PriorityQueue(max, f), PriorityQueue(max, f))
    #sol = ilp_lattice_search(problem, PQueue_heapq(max, f), PQueue_heapq(max, f))
    return sol
