...
class DrawingFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title, fileName=None):
        ...
        #wx.EVT_TIMER(self, menu_TIMER, self.onNextFrame) 
        ...
        topSizer.Add(self.infoPanel, 1, wx.EXPAND)
    ...
    def loadContents(self):
        ...
        try:
            ...
            for obj_name in objData:
                obj_class = objData[obj_name][0]
                obj_data  = objData[obj_name][1]                
                ...
                obj = obj_class(name, position, size, penColour, fillColour, lineSize)
    ...
    def saveContents(self):
        ...
        try:
            objData = {}
            for obj in self.contents:
                obj_name = obj.getData()[-1]
                objData[obj_name] = [obj.__class__, obj.getData()]
    ...
    def newsaveContents(self):
        ...
        try:
            ...
            for frame in self.frame_data:
                episodeData[frame] ={}
                for obj in self.frame_data[frame]['contents']:
                    obj_name = obj.getData()[-1]
                    episodeData[frame][obj_name] = [obj.__class__, obj.getData()]

            f = open(self.fileName, "wb")
            cPickle.dump(episodeData, f)
            f.close()
...
class RectDrawingObject(DrawingObject):
    ...
    def _privateDraw(self, dc, position, selected):
        ...
        dc.DrawRectangle(position.x, position.y,
                         self.size.width, self.size.height)
        dc.DrawText(repr(self.name), position.x+3, position.y+3)
