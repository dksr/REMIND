...
# MA 02110-1301,sys.path.append('/home/csunix/scksrd/work/cof_learn/lib/python2.5/site-packages/')
import sys
sys.path.append('/home/csunix/visdata/cofriend/ext_lib/lib/python2.5/site-packages/')
    ...
    def __str__(self):
        if self.name is not None and self.arity is not None:
            ...
            for arg in self.args:
                if type(arg) in (Atom, Variable):
                    arg_str.append(arg.chars)
                elif type(arg) is str:
                    arg_str.append(arg)
                elif type(arg) is Functor:
                    arg_str.append(str(arg))                 
                else:
                    arg_str.append(repr(arg))
            return "%s(%s)" % (self.name.chars,",".join(arg_str))
        else:
            return self.__repr__()

    def __repr__(self):
        return "".join(["Functor(", ",".join(str(x) for x in [self.handle,self.name.chars,self.arity]+self.args), ")"])
...
def _unifier(arity, *args):
    ...
    #    args[0].unify(args[1])
    try:
        # Should be PL_is_functor(args[1]), but gives some error.
