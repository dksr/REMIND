...
ver = '5.10.1'
host = os.uname()[1]
if 'leeds.ac.uk' in host:
    if 'cslin' not in host:
        # Running in arc1 computer
        local_swi_dir = '/home/ufaserv1_f/scksrd/ext_lib/swi-' + ver
    else:
        local_swi_dir = os.path.join(os.environ['COF_LEARN_ROOT'], 'ext_lib')
else: 
    local_swi_dir = os.path.join(os.environ['COF_LEARN_ROOT'], 'ext_lib')
...
swi_ver = 'swipl-' + ver
if 'i686' in arch:
    local_libpl_so_file = os.path.join(local_swi_dir, 'lib', swi_ver, 'lib/i686-linux/libswipl.so')
elif 'x86_64' in arch:
    local_libpl_so_file = os.path.join(local_swi_dir, 'lib', swi_ver, 'lib/x86_64-linux/libswipl.so')

cof_swi_dir = "/home/csunix/visdata/cofriend/ext_lib/swi/"
cof_libpl_so_file = cof_swi_dir + "lib/swipl-5.10.1/lib/x86_64-linux/libswipl.so"
...
try:
    if sys.platform[:3] == "win":
        ...
        # UNIX-like
        try:
            _lib = CDLL(local_libpl_so_file)
        except IndexError:
            # let's try the cof dir
            _lib = CDLL(cof_libpl_so_file)
    ...
    #print>>sys.stderr, "1) SWI-Prolog not installed as a shared library. Install SWI-Prolog (5.8.0 works just fine)"
    sys.exit(1)   
