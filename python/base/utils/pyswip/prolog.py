...
def _initialize():
    ...
    plargs[1] = "-q"
    plargs[2] = "-nosignals"
    plargs[3] = "-L1024M"  # <-- added
    plargs[4] = "-G2560M"  # <-- added
    PL_initialise(5, plargs)  # <-- modified, no. of args 
...
from pyswip.easy import getTerm, Functor
...
class Prolog:
    ...
    class _QueryWrapper(object):
        __slots__ = "swipl_fid","swipl_qid","error","log"
        
        def __init__(self):
            self.error = False
            #self.log = logging.getLogger("base.ilp.prolog")
        ...
        def __del__(self):
            if not self.error:
                ...
                PL_discard_foreign_frame(self.swipl_fid)    
                #apm = psutil.avail_phymem()#/(1024)
                #self.log.info(GBT + 'AVAIL_PHY_MEM: ' + BBB + repr(apm) + RESET)
